...
class InvoicePrinter extends FPDF
{
    ...
    public const ICONV_CHARSET_OUTPUT_A = 'ISO-8859-1';
    public const ICONV_CHARSET_OUTPUT_B = 'windows-1252';
...
if (isset($this->title) && !empty($this->title)) {
    $title = mb_strtoupper($this->title, 'UTF-8');
...
$positionX = $this->document['w'] - $this->margins['l'] - $this->margins['r']
             - max(
                 $this->GetStringWidth(mb_strtoupper($this->lang['number'], 'UTF-8')),
                 $this->GetStringWidth(mb_strtoupper($this->lang['date'], 'UTF-8')),
                 $this->GetStringWidth(mb_strtoupper($this->lang['due'], 'UTF-8'))
             )
             - max(
                 $this->GetStringWidth(mb_strtoupper((string)$this->reference, 'UTF-8')),
                 $this->GetStringWidth(mb_strtoupper((string)$this->date, 'UTF-8'))
             );
...
if (!empty($this->reference)) {
    ...
    $numberLabel = mb_strtoupper($this->lang['number'], 'UTF-8') . ':';
    $this->Cell(32, $lineheight, $numberLabel, 0, 0, 'L');
...
$this->SetFont($this->font, 'B', 9);
$this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
$dateLabel = mb_strtoupper($this->lang['date'], 'UTF-8') . ':';
$this->Cell(32, $lineheight, $dateLabel, 0, 0, 'L');
$this->SetTextColor(50, 50, 50);
$this->SetFont($this->font, '', 9);
...
if (!empty($this->time)) {
    ...
    $this->Cell(
        32,
        $lineheight,
        mb_convert_encoding(mb_strtoupper($this->lang['time'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT) . ':',
        0,
        0,
        'L'
    );
...
if (!empty($this->due)) {
    ...
    $this->SetFont($this->font, 'B', 9);
    $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
    $this->Cell(32, $lineheight, mb_convert_encoding(mb_strtoupper($this->lang['due'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT) . ':', 0, 0, 'L');
    $this->SetTextColor(50, 50, 50);
    $this->SetFont($this->font, '', 9);
    $this->Cell(0, $lineheight, $this->due, 0, 1, 'R');
        ...
        if (count($this->customHeaders) > 0) {
            foreach ($this->customHeaders as $customHeader) {
                ...
                $this->SetFont($this->font, 'B', 9);
                $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
                $this->Cell(32, $lineheight, mb_convert_encoding(mb_strtoupper($customHeader['title'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT) . ':', 0, 0, 'L');                $this->SetTextColor(50, 50, 50);
                $this->SetFont($this->font, '', 9);
        ...
        if ($this->PageNo() == 1) {
            ...
            if ($this->display_tofrom === true) {
                if ($this->displayToFromHeaders === true) {
                    $this->Cell($width, $lineheight, mb_convert_encoding(mb_strtoupper($this->lang['from'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT), 0, 0, 'L');
                    $this->Cell(0, $lineheight, mb_convert_encoding(mb_strtoupper($this->lang['to'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT), 0, 0, 'L');                    
                    $this->Ln(7);
                ...
                $this->SetTextColor(50, 50, 50);
                $this->SetFont($this->font, 'B', 10);
                $this->Cell($width, $lineheight, mb_convert_encoding($this->from[0] ?? '', self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT), 0, 0, 'L');
                $this->Cell(0, $lineheight, mb_convert_encoding($this->to[0] ?? '', self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT), 0, 0, 'L');                $this->SetFont($this->font, '', 8);
                ...
                for ($i = 1, $iMax = max($this->from === null ? 0 : count($this->from), $this->to === null ? 0 : count($this->to)); $i < $iMax; $i++) {
                    ...
                    if (!empty($this->from[$i]) || !empty($this->to[$i])) {
                        $this->Cell($width, $lineheight, mb_convert_encoding(empty($this->from[$i]) ? '' : $this->from[$i], self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT), 0, 0, 'L');                        
                        $this->Cell(0, $lineheight, mb_convert_encoding(empty($this->to[$i]) ? '' : $this->to[$i], self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT), 0, 0, 'L');                    
                    }
                    $this->Ln(5);
                }
                $this->Ln(-6);
        ...
        if (!isset($this->productsEnded)) {
            ...
            $this->Cell(
                ...
                10,
                mb_convert_encoding(mb_strtoupper($this->lang['product'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT),                0,
                0,
                'L',
                0
            );
            ...
            if (isset($this->quantityField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                    mb_convert_encoding(mb_strtoupper($this->lang['qty'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT),                    0,
                    0,
                    'C',
                    0
                );
            ...
            if (isset($this->vatField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                    mb_convert_encoding(mb_strtoupper($this->lang['vat'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT),                    0,
                    0,
                    'C',
                    0
                );
            ...
            if (isset($this->priceField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                    mb_convert_encoding(mb_strtoupper($this->lang['price'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT),                    0,
                    0,
                    'C',
                    0
                );
            ...
            if (isset($this->discountField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                    mb_convert_encoding(mb_strtoupper($this->lang['discount'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT),                    0,
                    0,
                    'C',
                    0
                );
            ...
            if (isset($this->totalField)) {
                ...
                $this->Cell(
                    $width_other,
                    10,
                    mb_convert_encoding(mb_strtoupper($this->lang['total'], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT),                    0,
                    0,
                    'C',
                    0
                );
    ...
    public function Body()
    {
            ...
            foreach ($this->items as $item) {
                ...
                if ($item['description']) {
                    ...
                    $calculateHeight->MultiCell(
                        $this->firstColumnWidth,
                        3,
                        mb_convert_encoding($item['description'], self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT),                        0,
                        'L',
                        1
                    );
                ...
                $this->Cell(
                    $this->firstColumnWidth,
                    $cHeight,
                    mb_convert_encoding($item['item'], self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT),                    0,
                    0,
                    'L',
                    1
                );
                ...
                if (isset($this->vatField)) {
                    $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                    if (isset($item['vat'])) {
                        $this->Cell($width_other, $cHeight, mb_convert_encoding($item['vat'], self::ICONV_CHARSET_OUTPUT_B, self::ICONV_CHARSET_INPUT), 0, 0, 'C', 1);                    } else {
                        $this->Cell($width_other, $cHeight, '', 0, 0, 'C', 1);
                    }
                }
                if (isset($this->priceField)) {
                    $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                    if (isset($item['price'])) {
                        $this->Cell($width_other, $cHeight, mb_convert_encoding($item['price'], self::ICONV_CHARSET_OUTPUT_B, self::ICONV_CHARSET_INPUT), 0, 0, 'C', 1);                    } else {
                        $this->Cell($width_other, $cHeight, '', 0, 0, 'C', 1);
                    }
                }
                if (isset($this->discountField)) {
                    ...
                    if (isset($item['discount'])) {
                        $this->Cell($width_other, $cHeight, mb_convert_encoding($item['discount'], self::ICONV_CHARSET_OUTPUT_B, self::ICONV_CHARSET_INPUT), 0, 0, 'C', 1);                    } else {
                        $this->Cell($width_other, $cHeight, '', 0, 0, 'C', 1);
                    }
                }
                if (isset($this->totalField)) {
                    $this->Cell($this->columnSpacing, $cHeight, '', 0, 0, 'L', 0);
                    if (isset($item['total'])) {
                        $this->Cell($width_other, $cHeight, mb_convert_encoding($item['total'], self::ICONV_CHARSET_OUTPUT_B, self::ICONV_CHARSET_INPUT), 0, 0, 'C', 1);                    } else {
                        $this->Cell($width_other, $cHeight, '', 0, 0, 'C', 1);
                    }
                }
                $this->Ln();
            ...
            if ($this->totalsAlignment == 'horizontal') {
                ...
                for ($i=0;$i<$totalsCount;$i++) {
                    $this->Cell(
                        ...
                        7,
                        mb_convert_encoding($this->totals[$i]['name'], self::ICONV_CHARSET_OUTPUT_B, self::ICONV_CHARSET_INPUT),                        1,
                        0,
                        'C',
                        true
                    );
                ...
                for ($y=0;$y<$totalsCount;$y++) {
                    $this->Cell(
                        ...
                        6,
                        mb_convert_encoding($this->totals[$y]['value'], self::ICONV_CHARSET_OUTPUT_B, self::ICONV_CHARSET_INPUT),                        'LRB',
                        0,
                        'C',
                ...
                foreach ($this->totals as $total) {
                    ...
                    $this->Cell(
                        $width_other - 1,
                        $cellHeight,
                        mb_convert_encoding($total['name'], self::ICONV_CHARSET_OUTPUT_B, self::ICONV_CHARSET_INPUT),                        0,
                        0,
                        'L',
                        1
                    );
                    ...
                    $this->Cell($width_other, $cellHeight, mb_convert_encoding($total['value'], self::ICONV_CHARSET_OUTPUT_B, self::ICONV_CHARSET_INPUT), 0, 0, 'C', 1);                    $this->Ln();
                    $this->Ln($this->columnSpacing);
                }
            }
        }
        ...
        if ($this->badge) {
            ...
            $this->Write(10, mb_convert_encoding(mb_strtoupper($badge, self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_B, self::ICONV_CHARSET_INPUT));            $this->Rotate(0);
        ...
        foreach ($this->addText as $text) {
            if ($text[0] == 'title') {
                $this->SetFont($this->font, 'b', 9);
                $this->SetTextColor(50, 50, 50);
                $this->Cell(0, 10, mb_convert_encoding(mb_strtoupper($text[1], self::ICONV_CHARSET_INPUT), self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT), 0, 0, 'L', 0);                $this->Ln();
                $this->SetLineWidth(0.3);
                $this->SetDrawColor($this->color[0], $this->color[1], $this->color[2]);
                $this->Line(
                    $this->margins['l'],
            ...
            if ($text[0] == 'paragraph') {
                ...
                $this->SetFont($this->font, '', 8);
                $this->MultiCell(0, 4, mb_convert_encoding($text[1], self::ICONV_CHARSET_OUTPUT_A, self::ICONV_CHARSET_INPUT), 0, 'L', 0);                $this->Ln(4);
            }
        }
    }
    ...
    public function Footer()
    {
        ...
        $this->Cell(
            0,
            10,
            mb_convert_encoding($this->lang['page'], 'ISO-8859-1', 'UTF-8') . ' ' . $this->PageNo() . ' ' . mb_convert_encoding($this->lang['page_of'], 'ISO-8859-1', 'UTF-8') . ' {nb}',            0,
            0,
            'R'
        );
    }
